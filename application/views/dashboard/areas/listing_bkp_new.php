						...
						<div class="panel-body" id="sortData" name="sortData" >
							<table id="datatable_state" class="table table-striped table-bordered table-hover" >
								...
								<tbody id ="userListing">
									<?php if(isset($alldata) && valResultSet($alldata)) { 
									$i = 1;
...
$(document).ready(function() {	function callback(data) {
		return data;
	}
	function format ( tablename,parent_name,parent_id ,callback ) {
		alert(tablename);
		/* alert(parent_name);
		alert(parent_id); */
			...
			currentRequest = jQuery.ajax({
				type: 'POST',
				url: DASHURL + "/areas/ajaxlisting/"+tablename+"/"+parent_id,
				beforeSend : function()    {           
	...
	<?php if($tablename == "state"){ ?>
		...
		$('#datatable_<?php echo $tablename; ?>').on('click', 'td.show_city', function () {
					...
					if (row.child.isShown()) {
						...
						format('city','<?php echo $tablename; ?>',state_id);
